...
Connection.prototype.sendBlock = function(block, txs) {
  ...
  this.sendMessage('mblk', put.buffer()); //PEER DISCONNECTION FIX
};
...
Connection.prototype.parseMessage = function(command, payload) {
  ...
  switch (command) {
    case 'version': // https://en.bitcoin.it/wiki/Protocol_specification#version
	  ...
	  data.addr_me = parser.buffer(30); // Crash Correction
      data.addr_you = parser.buffer(30); // Crash Correction
    ...
    case 'block':
	case 'mblk': //PEER DISCONNECTION FIX
      var block = new Block();
      block.parse(parser);
